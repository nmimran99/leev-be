...
export const getDistributionList = (data, type, actionBy) => {
	return new Promise(async (resolve, reject) => {
		if (type === "relatedUserAdded") {
			...
			let dist = [...data.fullDocument.relatedUsers, data.fullDocument.owner];
			if (data.fullDocument.system) {
				let system = await System.findOne({ _id: data.fullDocument.system });
				dist = [...dist, ...system.relatedUsers];
				dist.push(system.owner);
			}
			if (data.fullDocument.asset) {
				let asset = await Asset.findOne({ _id: data.fullDocument.asset });
				dist.push(asset.owner);
			}
			dist = vetDistList(dist, actionBy ? [actionBy] : null);
			resolve(dist);
		}
	});
};
	...
	try {
		...
		if (["faults", "tasks"].includes(objectType)) {
			...
			if (n.actionType === "statusChange") {
				...
				context.statusText = `${getFullName(actionBy)} ${t(
					`email.changeStatusText`
				)}`;
				context.statusColor = `background: ${getStatusColor(
					item.status.statusId
				)}`;
				emailSubject = `${externalId} -  ${t(
					`email.${n.actionType}Subject`
				)} - ${t(`statuses.${item.status.statusId}`)}`;
			}

			if (n.actionType === "ownerChange" || "itemCreated") {
				context.ownerName = getFullName(item.owner);
				context.ownerText = `${getFullName(actionBy)} ${t(
					`email.changeOwnerText`
				)}`;
			...
			if (n.actionType === "addComment") {
				context.commentText = `${getFullName(actionBy)} ${t(
					"email.addCommentText"
				)}`;
				context.comment = n.data.commentText;
				emailSubject = `${externalId} -  ${t(
					`email.${n.actionType}Subject`
				)} - ${getFullName(actionBy)}`;
			}

			if (n.actionType === "relatedUserAdded") {
				context.relatedUserAddedText = `${getFullName(actionBy)} ${t(
					"email.relatedUserAddedText"
				)}`;
				context.avatar = actionBy.avatar;
				emailSubject = `${externalId} -  ${t(
					`email.${n.actionType}Subject`
			...
			if (n.actionType === "itemCreated") {
				...
				context.itemCreatedSuccessfulyText =
					actionBy.email === "system@leev.co.il"
						? t(`email.${objectType}CreatedSuccessfulyTextSystem`)
						: `${getFullName(actionBy)}  ${t(
								`email.${objectType}CreatedSuccessfulyText`
						  )}`;
				context.itemCreatedSuccessfulyInstructions = t(
					"email.itemCreatedSuccessfulyInstructions"
				);
				emailSubject = `${externalId} -  ${t(
					`email.${objectType}AssignEmailSubject`
				)}`;
			}
		}
